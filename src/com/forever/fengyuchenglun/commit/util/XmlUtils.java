...
public class XmlUtils {
     ...
     * @return the values
     */
    public static <T extends Enum<T>> Set<T> getValues(Element element, String childName, Class<T> type) {
        Set<T> result = new LinkedHashSet<T>();
        for (Object value : element.getChildren(childName)) {
            if (value instanceof Element) {
                Element elem = (Element) value;
                String name = elem.getValue();
                result.add(Enum.valueOf(type, name));
     ...
     * @return the values
     */
    public static Set<Element> getValues(Element element, String childName) {
        Set<Element> result = new LinkedHashSet<Element>();
        for (Object value : element.getChildren(childName)) {
            if (value instanceof Element) {
                result.add((Element) value);
